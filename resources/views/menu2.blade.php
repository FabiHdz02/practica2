...
<body class="d-flex flex-column min-vh-100">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            // Obtener el estado inicial desde el servidor
            fetch('{{ route('get.estado') }}')
                .then(response => response.json())
                .then(data => {
                    toggleButton.textContent = data.estado === 'opened'
                        ? 'Terminar la Apertura'
                        : 'Iniciar Apertura de los Alumnos';
                });
   
            // Cambiar el estado al hacer clic
            toggleButton.addEventListener('click', () => {
                const newState = toggleButton.textContent === 'Iniciar Apertura de los Alumnos'
                    ? 'opened'
                    : 'closed';
   
                // Actualizar el texto del botón
                toggleButton.textContent = newState === 'opened'
                    ? 'Terminar la Apertura'
                    : 'Iniciar Apertura de los Alumnos';
   
                // Enviar el nuevo estado al servidor
                fetch('{{ route('update.estado') }}', {
                    method: 'POST',
                    headers: {
